...
export async function fetchFromServer(pokemon: string) {
  clearSearch();
  try {
    let response = await fetch(`https://pokemon-eden-ariel.herokuapp.com/pokemon/${pokemon.toLowerCase()}/`);
    if (response.status == 404 || pokemon === "#") {
      // error
      throw "No pokemon matched your search!";
    }
    let json = await response.json();
    // json.abilities.replace(/\]|\[/g, "").split(",")
    //unhide
    const pokemonDiv = document.getElementById("pokemon-div") as HTMLDivElement;
    pokemonDiv.style.display = "";
    //name
    const name = document.getElementById("name");
    name!.innerHTML = `${json.name.charAt(0).toUpperCase()}${json.name.substring(1)}`;
    //front image
    const frontImg = document.getElementById("frontImg");
    const frontImageAttribute = json.front_image || "./noImage.png";
    frontImg!.setAttribute("src", frontImageAttribute);
    //back image
    const backImg = document.getElementById("backImg");
    const backImageAttribute = json.back_image || "./noImage.png";
    ...
    for (let eachAbility of json.abilities.replace(/\]|\[/g, "").split(",")) {
    ...
    for (let eachType of json.types.replace(/\]|\[/g, "").split(",")) {
    ...
    for (let stat of json.stats.replace(/\]|\[/g, "").split(",")) {
