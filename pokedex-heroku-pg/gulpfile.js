...
gulp.task("scss", () => {
  return gulp.src("./src/client/**/*.scss").pipe(sass()).pipe(gulp.dest("./dist"));
});
...
// Transfers index
gulp.task("icon", () => {
  return gulp.src(["./src/client/favicon.ico", "./src/client/noImage.png"]).pipe(gulp.dest("./dist"));
});
...
// Build all
gulp.task("build", gulp.series("clean", "scss", "index", "icon", "tsc", "bundle"));
...
gulp.task("heroku-copy-root", () => {
  return gulp
    .src(["./package.json", "./package-lock.json", "./Procfile", "./dist/tsc/server/server.js"])
...
// Heroku deploy
gulp.task("deploy", gulp.series("heroku-clean", "build", "heroku-copy-root", "heroku-copy-dist"));

// Run all (without express)
gulp.task("dev", gulp.series("build", gulp.parallel("watch-scss", "watch-html", "watch-tsc", "tsc-w")));

// Run all together
gulp.task("default", gulp.series("build", gulp.parallel("watch-scss", "watch-html", "watch-tsc", "tsc-w", "express")));
